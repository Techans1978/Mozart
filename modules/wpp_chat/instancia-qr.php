...
if ($token) {
    ...
    // >>> de acordo com a doc: start-session pode RETORNAR o QR se waitQrCode=true
    $startPayload = [
        'webhook'    => null,
        'waitQrCode' => true,
    ];
...
if ($token) {
    ...
    $connected = (bool)($status['json']['connected'] ?? false);

    // Se NÃO estiver conectado, precisamos de QR
    if (!$connected) {
        // 3.1) PRIMEIRO: tentar pegar o QR da resposta do start-session (doc diz que ele retorna o QR)
        $qrcodeRaw = null;

        if (isset($start['json']['qrcode'])) {
            // pode vir como string ou como objeto
            if (is_array($start['json']['qrcode'])) {
                $qrcodeRaw =
                    $start['json']['qrcode']['base64'] ??
                    $start['json']['qrcode']['qrcode'] ??
                    null;
            } else {
                $qrcodeRaw = $start['json']['qrcode'];
            }
        }

        if (!$qrcodeRaw && isset($start['json']['base64'])) {
            $qrcodeRaw = $start['json']['base64'];
        }

        if ($qrcodeRaw) {
            $qr_debug = $start; // para debug no HTML
        } else {
            // 3.2) Se o start-session não trouxe QR, cai para /qrcode-session (fluxo antigo)
        ...
        // 3.3) Normalizar o que achamos (seja do start ou do qrcode-session)
        if ($qrcodeRaw) {
            if (strpos($qrcodeRaw, 'data:image') === 0) {
                ...
                // veio algum outro texto (urlCode, etc.) -> força para PNG
                $qrDataUrl = 'data:image/png;base64,' . base64_encode($qrcodeRaw);
                $qr_note   = 'QR veio em formato não reconhecido como base64; convertido para PNG.';
            }
        } else {
            // ainda não? tenta heurísticas no corpo bruto da resposta do /qrcode-session
            if ($qr) {
...
<?php endif; ?>
  ...
  <div class="wpp-card">
    ...
    <?php $connected = $connected ?? (bool)($status['json']['connected'] ?? false); ?>
<p>
  Conectado: <?= $connected ? '<span class="ok">SIM ✅</span>' : '<span class="err">NÃO ❌</span>' ?>
</p>
