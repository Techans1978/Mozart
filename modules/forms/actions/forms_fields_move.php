<?php
// modules/forms/actions/forms_fields_move.php — move field para cima/baixo na seção no schema_json (draft)
...
$dir = $_POST['dir'] ?? '';
if ($formId<=0 || $sectionId==='' || $fieldId==='' || !in_array($dir,['up','down'],true)){
  flash('Ação inválida.'); go($formId,$sectionId);
}
...
for ($i=0;$i<count($fields);$i++){
  if ((string)($fields[$i]['id'] ?? '') === $fieldId){ $idx=$i; break; }
}
if ($idx<0){ flash('Campo não encontrado.'); go($formId,$sectionId); }

$newIdx = ($dir==='up') ? $idx-1 : $idx+1;
if ($newIdx < 0 || $newIdx >= count($fields)) { go($formId,$sectionId); }
