<?php
// modules/forms/actions/forms_rules_delete.php
...
if ($formId<=0 || $ruleId===''){ flash('Dados inválidos.'); go($formId); }
...
$before = count($schema['rules']);
$schema['rules'] = array_values(array_filter($schema['rules'], function($r) use ($ruleId){
  return (string)($r['id'] ?? '') !== $ruleId;
}));
$after = count($schema['rules']);
...
flash(($after<$before) ? 'Regra excluída.' : 'Regra não encontrada.');
go($formId);
