...
$stmt = $conn->prepare("UPDATE forms_form SET title=?, description=?, tags=? WHERE id=? LIMIT 1");
$stmt->bind_param("sssi",$title,$description,$tags,$id);
$stmt->execute();
$stmt->close();

flash('Formulário atualizado.');
header('Location: '.BASE_URL.'/public/modules/forms/forms_editar.php?id='.$id);
