...
if (empty($nome_completo) || empty($cpf) || empty($nome_fantasia) || empty($celular)) {
    header("Location: erro-campos.html?mensagem=" . urlencode("Por favor, preencha todos os campos obrigatórios."));
    exit(); // Certifique-se de sair após o redirecionamento
}

if (!preg_match('/^\d{11}$/', $cpf)) {
    header("Location: erro-cpf.html?mensagem=" . urlencode("CPF inválido! Deve ser colocado apenas números. Deve conter 11 dígitos numéricos."));
    exit();
}

if ($tem_agencia && empty($nome_agencia)) {
    header("Location: erro-nome-agencia.html?mensagem=" . urlencode("Por favor, preencha o nome da agência."));
    exit();
}
...
if ($stmt_check_lista_fornecedores->num_rows === 0) {
    header("Location: erro-fornecedor.html?mensagem=" . urlencode("Fornecedor não encontrado no banco de dados."));
    exit();
}
...
if ($stmt_check->num_rows > 0) {
    header("Location: erro-cpf-cadastrado.html?mensagem=" . urlencode("CPF já cadastrado!"));
    exit();
} else {
    ...
    if ($stmt->execute()) {
        header("Location: ok-sucesso.html?mensagem=" . urlencode("Dados salvos com sucesso!"));
        exit();
    } else {
